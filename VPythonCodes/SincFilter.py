...
# SincFilter.py: VPthon, Noise reduction by filtering
...
graph2 = graph(x=0, y=250, width=600, height=250,
               title='Signal+Noise', xtitle='t (s)', ytitle='y(t)',
               xmin=0, xmax=25, ymin=0, ymax=10)
funct2 = gcurve(color=color.red)
graph3 = graph(x=0, y=500, width=600, height=250,
               title='Filtered Input', xtitle='t (s)', ytitle='y(t)',
               xmin=0, xmax=25, ymin=0, ymax=10)
...
def function(array, max):
    ...
    for i in range(0, max):
        f[i] = 1 / (1. - 0.9 * sin(x))          # Function, then noise
...
def filter():                    # Low-pass windowed sinc filter
    ...
    for j in range(0, max - 1):
        funct3.plot(pos=vector(j * step, y[j]))
