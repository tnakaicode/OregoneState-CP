...
Rcurve = curve(color=color.red,radius=2)               
Gcurve = curve(color=color.green,radius=2)          
Bcurve = curve(color=color.blue,radius=2)            
title  = label(pos=vector(-20,350,0),
	text='Intensity vs Thickness (nA)',box=0)
waves  = label(pos=vector(-30,320,0), 
	text='Red, Green, and Blue Intensities',box=0)
trans  = label(pos=vector(-280,300,0),text='Transmission',box=0)
refl   = label(pos=vector(210,300,0),text='Reflection',box=0)
lamR   = 572;   lamB = 430; lamG = 540; i = 0    # R,B, G 
film = curve(pos=[(-150,-250,0),(150,-250,0),(150,250,0),
          (-150,250,0),(-150,-250,0)])
Rc = [];  Gc = []; Bc = []                    # R,G,B intensity
...
ap = -500*nA/1240 +250       
Rc = Rc+[intR];  Gc = Gc+[intG];  Bc = Bc+[intB]      
Rt = [];  Gt = [];      Bt = []
DelRt = 4*pi*nA/lamR;   DelGt = 4*pi*nA/lamG; DelBt = 4*pi*nA/lamB
IntRt = cos(DelRt/2)**2; IntGt = cos(DelGt/2)**2
IntBt = cos(DelBt/2)**2     
...
for nA in range (0,125):    
  ...
  box(pos=vector(205,reflesc,0),width=0.1,height=10,
  	  length=50,color=col)
  colt = vector(IntRt[nA],IntGt[nA],IntBt[nA]) 
  box(pos=vector(-270,reflesc,0),width=0.1,height=10,
  	  length=50,color=colt) 
  if (nA%20==0):                           
     prof = nA*10
