...
# QuantumEigenCallVP.py: VPthon, E & psi via rk4 + bisection
...
def plot(h):                   # Repeat integrations for plot
    ...
    for ix in range(Nsteps -1,nL + 2,-1):        # Right WF
      ...
      y = rk4Algor(x, -h, 2, y, f) 
      Rwf.append(pos=vector(2.*(ix+1-Nsteps/2)-500, y[0]*35e-9+200,0))
      j += 1
    x = x-h              
    normL = y[0]/yL[0][nL] 
    for ix in range(0,nL+1):   # Normalize L wf & derivative
      ...
      Lwf.append(pos=vector(2.*(ix-Nsteps/2+1)-500, y[0]*35e-9+200,0))
