...
scenePE = graph(width=400, height=150, title='Pot Energy', ymin=-0.6, ymax=0.0, xmin=0, xmax=100,
...
def Forces(t, PE):                                   # Forces
    ...
    for i in range(0, Natom - 1):
        for j in range(i + 1, Natom):
            dx = x[i] - x[j]
            if (abs(dx) > L / 2):
                dx = dx - sign(L, dx)  # Image
            r2 = dx * dx
            if (r2 < r2cut):
                if (r2 == 0.):
                    r2 = 0.0001   # Avoid 0 denom
                invr2 = 1. / r2
...
def timevolution():
    ...
    while t < 100:                              # Time loop
        ...
        t1 = t2
        t2 = Itemp
        Kavegraph.plot(pos=vector(t, KE, 0))                # Plot KE
        PEcurve.plot(pos=vector(t, PE, 0), display=scenePE)  # Plot PE
        t += 1
