...
# MDpBC.py: VPython MD with Periodic BC
...
border = curve(pos=[(-L, -L, 0), (L, -L, 0),
                    (L, L, 0), (-L, L, 0), (-L, -L, 0)])
half = curve(pos=[(0, -L, 0), (0, L, 0)], color=color.yellow)  # middle
ndist = graph(ymax=200, width=400, height=300,
              xtitle='Particles in right half', ytitle='N')
...
for i in range(0, Natom):           # initial x's and v's
    col = vec(1.3 * random.random(), 1.3 * random.random(),
              1.3 * random.random())
    x = 2. * (L - Ratom) * random.random() - L + Ratom     # positions
...
for t in range(0, 1000):
    Nrhs = 0    # begin at zero in ea time
