...
# QMC.py: VPthon, Quantum MonteCarlo, Feynman path integration
...
def trjaxs():                 # Plot axis for trajectories
    trax = curve(pos=[(- 97, -100, 0), (100, -100, 0)],
                 color=color.cyan, canvas=trajec)
    curve(pos=[(0, - 100, 0), (0, 100, 0)],
          color=color.cyan, canvas=trajec)
    label(pos=vector(0, -110, 0), text='0', box=0, canvas=trajec)
    label(pos=vector(60, -110, 0), text='x', box=0, canvas=trajec)


wvgraph = canvas(x=340, y=150, width=500, height=300,
                 title='Ground State Probability')
wvplot = curve(x=range(0, 100), canvas=wvgraph)   # Probability
wvfax = curve(color=color.cyan)


def wvfaxs():
    wvfax = curve(pos=[(-600, -155, 0), (800, -155, 0)],
