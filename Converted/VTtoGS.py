...
# VPython objects:
prims = ['arrow', 'box', 'cone', 'curve', 'cylinder', 'canvas',
...
primtypes = {'arrow': 'rect', 'box': 'rect', 'cone': 'radial', 'curve': 'curve', 'cylinder': 'radial',
             ...
             'text': 'not_implemented', 'local_light': 'light', 'distant_light': 'light', 'canvas': 'canvas'}

# variables which refer to VPython objects, in form var:prim
symbols = {}  
...
# (x,y) -> vector(x,y,0)
                if commas == 1:  
                    line = line[:start - 1] + 'vector' + 
                        line[start - 1:i - 1] + ',0' + line[i - 1:]
                    i += 8
# (x,y,z) -> vector(x,y,z)
                elif commas == 2:  
...
def firstpass(pname):
    ...
    output = []
# predefined name of a primitive
    symbols = {'scene': 'canvas'}  
    ...
    def replacements(line):
        line = line.replace('vector(0,0,0)', 'vector(0,0,0)')
        line = line.replace('waitfor('click')', "waitfor('click')")
        line = line.replace('asin', 'asin')
        line = line.replace('acos', 'acos')
        line = line.replace('atan', 'atan')
        line = line.replace('graph', 'graph')
        line = line.replace('canvas', 'canvas')
...
# location of '(' after primitive name
                begin = m.start(2) + 1  
                foundprim = True
                break
        ...
        if len(comments) > 0:
            # delete final carriage return, which add will attach
            add(comments[:-1])
        ...
        while True:
            ...
            if m:
                ...
                if name in symbols:
                    ...
                    if (attr in attrs) and (prim != 'curve') and (prim != 'canvas'):
                        newattr = attrs[attr]
                        line = line[:m.start(1) + start] + name + 
                            '.' + newattr + line[m.end(2) + start:]
