...
public class TwoDsol {
  ...
  public static void main(String[] argv) throws IOException, FileNotFoundException { 
    int nint;           // input positive integer proportional to time of wave packet
  ...
  public static void initial(double u[][][])  {
    ...
    int i, j, k; 
    dx = 14./200.; dy = dx;  dt = dx/Math.sqrt(2.);  dts = (dt/dx)*(dt/dx); 
    yy = -7.; time = 0.; 
    for ( i=0; i <= D-1; i++ )  {
      xx = -7.; 
      for ( j=0; j <= D-1; j++ )  { tmp = 3.-Math.sqrt(xx*xx + yy*yy); 
                                    u[i][j][0] = 4.*Math.atan(tmp); xx = xx + dx; }
      yy = yy + dy; 
    }
  }
  ...
  public static void solution(double u[][][], int nint) 
    ...
    PrintWriter w = new PrintWriter(new FileOutputStream("2Dsol.dat"), true);
    double dx, dy, dt, time, a2, zz, dts, a1, tmp; 
    int l, m, mm, k, j, i; 
    dx = 14./200.; dy = dx;  dt = dx/Math.sqrt(2.); 
    time = 0.; time = time + dt;  dts = (dt/dx)*(dt/dx);  tmp = 0.; 
    for ( m=1; m <= D-2; m++ )  {
      for ( l=1; l <= D-2; l++ )  {
    ...
    for ( k=0; k <= nint; k++ )  {                            // Following iterations
      ...
      if (k==nint) {
        for ( i=0; i <= D-1; i=i + 5)  {
          for (j=0; j <= D-1; j=j+5)  w.println(""+(float)Math.sin(u[i][j][2]/2)+"");
          w.println("  "); 
        }
      }
      time = time + dt;         
