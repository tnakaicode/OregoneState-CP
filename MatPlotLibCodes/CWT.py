...
invtrgr = graph(x=0, y=200, width=600, height=200, title=
 'Inverted Transform',xmin=0,xmax=12,ymin=-20,ymax=20)
...
dTau =  W/noTau;  dS =  (W/iS)**(1./noS); # Tiny s for hi freq
...
for i in range( 0, noS):
    for j in range( 0, noTau):
        if Yn[i,j] > maxY or Yn[i,j] < -maxY: maxY=abs(Yn[i,j])                     
tau = iT
s = iS
print("normalize")      
for i in range( 0, noS):
