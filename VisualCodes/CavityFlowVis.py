...
# CavityFlowVis.py: Visual, Navier-Stokes equation for cavity flow
...
while (iter <= Niter):
    if iter % 100 == 0:
        obj.new_fig()
        obj.axs.contourf(u, cmap="jet")
        obj.SavePng(obj.tempname + "_{:04d}.png".format(iter))
        plt.close()
        print("Iteration Number", iter)
    Relax(iter)
    iter += 1

np.savetxt ("./CavityFlowVis.txt", u)
